function opt_lags = order_criterion_f_ar_d(dat, F, lagF, lagAR, c, h, ic)

% AIC = zeros(lagAR+1, lagF+1);
% HQ = zeros(lagAR+1, lagF+1);
% SC = zeros(lagAR+1, lagF+1);
...
%         AIC(p+1, m+1) = log(rss) + (p + m*r + c) * 2/T;
%         HQ(p+1, m+1) = log(rss) + (p + m*r + c) * 2 * (log(log(T)))/T;
%         SC(p+1, m+1) = log(rss) + (p + m*r + c) * log(T)/T;
