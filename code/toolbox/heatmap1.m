function [hImage, hText, hXText] = heatmap1(mat, xlab, ylab, textmat, varargin)
...
% set(gca,'Box','off')
grid = 0.04;
p = setHatch(p, mat, grid);

% Add colorbar if selected
set(gca,'Clipping','off')
grid = 0.05;
colbar(p, grid);
% p = addColorbar(p, mat, textmat);
...
if ~strcmp(p.GridLines,'none')
    ...
    %     end
    for i = 1:length(p.GridY)
        line('Parent',p.hAxes,'XData',xlim, 'YData', [p.GridY(i) p.GridY(i)]+.5, 'LineStyle', p.GridLines);
    end
end
end

% Add color bar
function p = addColorbar(p, mat, textmat)
...
%     l = nan(10,1);
%     for ii = 1:10
%         l(ii) = line([0.01*ii 0.01*ii], [0 0.999], 'color', 'k', 'parent', h_axes);
%     end
...
if isempty(ylab) % No ticks or labels
    ...
    set(p.hAxes,'YTick',ytick,'Ticklength', [0 0],'YTickLabel',ylab(ytick));
end
if p.IsGraphics2
...
if isempty(xlab)
    ...
    if p.IsGraphics2
        set(p.hAxes,'XTick',xtick,'Ticklength', [0 0],'XTickLabel',xlab(xtick),'XTickLabelRotation', p.TickAngle);
    else
        if p.TickAngle == 0
            set(p.hAxes,'XTick',xtick,'Ticklength', [0 0],'XTickLabel',xlab(xtick));
        else
...
function hXText = createXTicks(hAxes, tickAngle, xticks, xticklabels, texInterpreter)
    ...
    set(hAxes, 'XTick', xticks, 'Ticklength', [0 0], 'XTickLabel', '');
...
if ~isempty(axInfo.ylab)
    ...
    set(hAxes, 'YTick', yticks, 'Ticklength', [0 0], 'YTickLabel', ylabels(yticks));
end
...
if ~isempty(axInfo.xlab)    
    ...
    if ~isempty(axInfo.hXText) % Rotated X tick labels exist
        ...
        set(hAxes, 'XTick', xticks, 'Ticklength', [0 0], 'XTickLabel', xlabels(xticks));
...
% Create hatch on heatmap
function p = setHatch(p, mat, grid)
for ii = 1:size(mat,1)
    for jj = 1:size(mat,2)
        if(mat(ii,jj) > 0.02)
            p.hLine = line_hatch(jj, ii, grid, 1);
        elseif(mat(ii,jj) < 0)
%             p.hLine = line_hatch(jj, ii, grid, 0);
        end
    end
end
    function l = line_hatch(x, y, grid, pn)
        if(pn == 0)
            aux1 = y-0.5:grid:y+0.5;
            l = line([(x-0.5) *ones(1, length(aux1)); (x+0.5) *ones(1, length(aux1))], [aux1; aux1]);
            for kk = 1:length(l)
                l(kk).Color = [0.1 0.1 0.1];
                l(kk).LineStyle = '--';
                l(kk).LineWidth =  0.1;
            end
            % diag lines
%             aux1 = x+0.5:-grid:x-0.5;
%             aux2 = x-0.5:grid:x+0.5;
%             aux3 = y-0.5:grid:y+0.5;
%             aux4 = y+0.5:-grid:y-0.5;
%             l = line([(x-0.5)*ones(1, length(aux1)), aux2 ; aux1, (x+0.5)*ones(1, length(aux2))], ...
%                 [aux3, (y-0.5)*ones(1, length(aux3)); (y+0.5)*ones(1, length(aux3)),aux4], 'Color', 'k');
        else
            aux1 = x-0.5:grid:x+0.5;
            l = line([aux1; aux1], [(y-0.5) *ones(1, length(aux1)); (y+0.5) *ones(1, length(aux1))]);
            for kk = 1:length(l)
                l(kk).Color = [0.1 0.1 0.1];
                l(kk).LineStyle = '--';
                l(kk).LineWidth =  0.5;
...
%             aux4 = y+0.5:-grid:y-0.5;
%             l = line([(x-0.5)*ones(1, length(aux1)), aux2 ; aux1, (x+0.5)*ones(1, length(aux2))], ...
%                 [aux4, (y+0.5)*ones(1, length(aux3)); (y-0.5)*ones(1, length(aux4)),aux3], 'Color', 'k');
        end
    end
end
...
axMaxX  = 1.2*maxAx;
xlim([minAx,maxAx])
line([maxAx,maxAx],[minAy,maxAy],'color','k','linewidth',1);
%
cmap = flipud(p.Colormap);
nColors = size(cmap, 1);
%
...
cbarWidth   = (maxAx - minAx)*0.05; 
cbarHeight  = maxAy; % actually half height
cbarLeft    = maxAx + 0.35 * cbarWidth;
cbarRight   = cbarLeft + cbarWidth;
cbarX       = [cbarLeft,cbarRight,cbarRight,cbarLeft];
cbar_min = minAy+(maxAy - cbarHeight);
cbarSeg     = (cbarHeight - cbar_min)/nColors;
cbarY     = (cbarHeight - cbar_min);

%
% Create each of the patches in the colorbar.
for iColor = 1:nColors
patch(cbarX,[cbar_min+(iColor-1)*cbarSeg, cbar_min+(iColor-1)*cbarSeg, ...
    cbar_min+iColor*cbarSeg, cbar_min+iColor*cbarSeg], ...
    cmap(iColor,:),'edgecolor','none')
end
%
% Create a patch to make the border around the bar.
line([cbarLeft,cbarRight],[cbar_min,cbar_min],'color','k','linewidth',1);
line([cbarLeft,cbarRight],[cbarHeight,cbarHeight],'color','k','linewidth',1);
line([cbarLeft,cbarLeft],[cbar_min,cbarHeight],'color','k','linewidth',1);
line([cbarRight,cbarRight],[cbar_min,cbarHeight],'color','k','linewidth',1);
%
% Create grid lines.
c_se = cbarY / 10;
val = 1:-0.2:-1;
for ii = 1:11
   line([cbarRight-0.1,cbarRight],[cbar_min+(ii-1)*c_se,...
       cbar_min+(ii-1)*c_se],'color','k','linewidth',1);
   if(mod(val(ii),1) == 0)
       text(cbarRight+0.05, cbar_min+(ii-1)*c_se, sprintf('% 1.0f',val(ii))); 
   else
       text(cbarRight+0.05, cbar_min+(ii-1)*c_se, sprintf('% 1.1f',val(ii))); 
   end
end
% Draw the line on it
len = cbarLeft:grid:cbarRight;
for ii = 1:length(len)
    line([cbarLeft+(ii-1)*grid, cbarLeft+(ii-1)*grid], [cbar_min (cbarHeight+cbar_min-0.05)/2], 'color', 'k');
end
end
